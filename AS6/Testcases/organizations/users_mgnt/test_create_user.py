from AS6.Pages.Organizations.users_mgnt.users_org_mgnt_page import user_mgnt_moudle
...
class Test_create_user:
    ...
    def test_create_user_success(self, driver, clean_user_info):
        # try:
            data = \
                do_excel(filename="organizations/users_mgnt/creat_user.xlsx", sheetname="创建用户", minrow=2, maxrow=2,
                         mincol=1,
                         maxcol=11)[
                    0]  # 读取excle 数据
            user_mgnt_moudle(driver).create_user(username=data[0], showname=data[1], remark=data[2], mobile=data[4],
                                                 quota_space=data[9])
            # user_mgnt_moudle(driver).del_user()
