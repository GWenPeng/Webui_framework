...
def thrift_generate_path():
    """thrift生成文件路径"""
    return os.path.join(script_path(), "gen_py_tmp")
...
def thrift_tool_path():
    ...
    if platform.system() == "Windows":
        thrift_file_name = "thrift-0.13.0.exe"
    else:
        thrift_file_name = "thrift"
...
def generate_thrift_files():
    ...
    tf_files = os.listdir(scr_path)
    if platform.machine() == 'aarch64':
        thrift_bin_path = 'thrift'
    else:
    ...
    for tf_file in tf_files:
        if os.path.splitext(tf_file)[1] == ".thrift":
            ...
            args.append(os.path.join(scr_path, tf_file))
            print (args)
